<template>
  <b-row>
    <b-col cols="12">
      <h2>
        Sala: (TODO) - <b-btn size="sm" @click.stop="logout()">Logout</b-btn>
      </h2>
      <b-list-group class="panel-body" v-chat-scroll>
        <b-list-group-item v-for="item in chats" v-bind:key="item" class="chat">
      ...
      <b-form @submit="onSubmit" class="chat-form">
        <b-input-group prepend="Mensagem: ">
...
export default {
  ...
  methods: {
    logout () {
      this.socket.emit('save-message', { room: this.chat.room, nickname: this.chat.nickname, message: this.chat.nickname + ' saiu da zoera! :(', created_date: new Date() });
      this.$router.push({
        name: 'RoomList'
      })
    },
